...
protected:
  ...
  int winx;
  int winy;
  int border; // pixels to offset the drawing
  cv::RNG rng;
...
public:
  ...
  void overlay(DrawWindow *W, int outline = 0, int r = 0, int g = 0, int b = 0) {
	...
	if (color[0] != r || color[1] != g || color[2] != b) { // ABC: need to check color order
	  if (outline) {
	    ...
	    if (is_boundary) {
	      grid.at<cv::Vec3b>(cv::Point(i + border, j + border)) = color;
	    }
  ...
  void setPenColorRandom() { 
    setPenColor(rng.uniform(100,200),rng.uniform(100,200),rng.uniform(100,200)); 
    printf("Setting pen to %d %d %d\n",rng.uniform(100,200),rng.uniform(100,200),rng.uniform(100,200));
  }
  ...
  void addPolyPoint(int x, int y) {
    poly_points.push_back(cv::Point(x + border, y + border));
  }
  ...
  void drawPixel(cv::Point pt) {
    pt.x += border;
    pt.y += border;
    if (pt.x >= 0 && pt.y >= 0 && pt.x < grid.cols && pt.y < grid.rows) {
      grid.at<cv::Vec3b>(pt) = pen_color_vec;
    }
  }

  void drawPixel(int x, int y) {
    drawPixel(cv::Point(x, y));
  }
  ...
  cv::Vec3b getColor(int x, int y) {
    return grid.at<cv::Vec3b>(cv::Point(x + border, y + border));
  }
  ...
  void drawRegion(std::vector<cv::Point> pixels) { // could be changed to pass reference but I've not figured out how to access vector
    for (unsigned int i = 0; i < pixels.size(); i++) {
      ...
      p.x += border;
      p.y += border;
      // TODO: try p instead of pixels[i]
  ...
  void setMat(cv::Mat mat) {
    width = mat.cols;
    height = mat.rows;
    grid.release(); // maybe deallocated memory??
    grid = mat;
  }

  // draw a rectangle around the border
  void drawBorder(int indent = 4, int set_thickness=1) {
    int orig_thickness = pen_thickness;
    setLineThickness(set_thickness);
    if (indent > border) { indent = border; }
    drawRectangle(-indent, -indent, width + indent, height + indent, 0); // draw outline of canvas
    setLineThickness(orig_thickness);
  }

  void setup(int w, int h, std::string name, int hide_window = 0, int blackwhite = 0) {
    width = w;
    height = h;
    window_name = name;
    window_created = 0;
  ...
  // creates a drawable area of w,h with a border padding
  DrawWindow(int w, int h, int border_width, std::string name, int hide_window = 0, int blackwhite = 0) { // constructor
    border = border_width;
    setup(w + 2 * border, h + 2 * border,name, hide_window, blackwhite);
    width = w;
    height = h;
  }

  // creates a drawable area of w,h with no border
  DrawWindow(int w, int h, std::string name, int hide_window = 0, int blackwhite = 0) { // constructor
    border = 0;
    setup(w,h,name, hide_window, blackwhite);
  }
